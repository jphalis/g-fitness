...
def daily_table(context, day, width, width_slot, height, start=0, end=24, increment=30):
    """
...
def options(context, occurrence):
    ...
    if CHECK_EVENT_PERM_FUNC(occurrence.event, user) and CHECK_CALENDAR_PERM_FUNC(occurrence.event.calendar, user):
        ...
        # context['completed_event'] = reverse('completed_event', args=(occurrence.event.id,))
...
def hash_occurrence(occ):
    return '%s_%s' % (occ.start.strftime('%d%m%Y%I%M%S'), occ.event.id)
