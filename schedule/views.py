...
def create_or_edit_event(request, calendar_slug, event_id=None, next=None,
    ...
    if date:
        try:
            ...
            lesson_time = datetime.datetime(**date)
            lesson_time_start = lesson_time.strftime("%m/%d/%Y %I:%M %p")
            initial_data = {
                "start": start,
...
#     lesson_amount = request.user.lesson_current_amount
#     membership_instance, created = LessonCount.objects.get_or_create(user=request.user)
#     membership_lesson_update.send(membership_instance, new_lesson_count=user.lesson_current_amount - 1)
#     messages.success(request, "The appointment has been completed!")
