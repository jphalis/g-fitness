...
from .signals import membership_lesson_update, membership_lesson_completed, membership_lesson_canceled
...
class UserAddress(models.Model):
	...
	# address3 = models.CharField(max_length=120, null=True, blank=True) #international purposes
	...
	def get_address(self):
		return "%s, %s, %s, %s, %s, %s" %(self.name, self.address, self.city, self.state, self.country, self.zipcode)
...
def user_logged_in_signal(sender, signal, request, user, **kwargs):
	...
	if created:
		try:
			membership_obj.lesson_current_amount = new_lesson_count #amount of purchased lessons
			membership_obj.save()
			user.is_member = True
			user.save()
		except:
			membership_obj.lesson_current_amount = 0 #amount of purchased lessons
			membership_obj.save()
