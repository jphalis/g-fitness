...
def slider_upload(instance, filename):
	return "sliders/images/homepage/%s" %(filename)
	# if you have a user: return "images/homepage/slider/%s" %(instance.user, filename)
...
def media_upload(instance, filename):
	return "media/images/%s" %(filename)

class Media(models.Model):
	image = models.ImageField(upload_to=media_upload)
	order = models.IntegerField(default=0)
	redirect_url = models.CharField(max_length=250, null=True, blank=True)
	header_text = models.CharField(max_length=120, null=True, blank=True)
	text = models.CharField(max_length=120, null=True, blank=True)
